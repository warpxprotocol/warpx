...
pub mod pallet {
	...
	#[pallet::storage]
	#[pallet::unbounded]
	pub type OrderRecords<T: Config> = StorageDoubleMap<
		_,
		Twox64Concat,
		T::AccountId,
		Twox64Concat,
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_liquidity(
			...
			if base_asset_reserve.is_zero() || quote_asset_reserve.is_zero() {
				...
				let quote_asset_optimal =
					Self::quote(&base_asset_desired, &base_asset_reserve, &quote_asset_reserve)?;
		...
		pub fn cancel_order(
			...
			Self::do_cancel_order(cancel_type, &maybe_owner, &*base_asset, &*quote_asset)?;
			Ok(())
		}
	...
	impl<T: Config> Pallet<T> {
		...
		pub(crate) fn do_place_order(
			...
			OrderRecords::<T>::insert(maker, pool_id, OrderRecord::new(order_id, price, quantity));
		...
		pub(crate) fn do_cancel_order(
			...
			Pools::<T>::try_mutate(pool_id.clone(), |pool| -> DispatchResult {
				...
				let order_id = updated
					.cancel_order(cancel_type, owner, &pool_id)
		...
		pub(crate) fn do_match_order(
			...
			while remain_orders > Zero::zero() {
				...
				let maybe_next_order =
					if is_bid { pool.next_ask_order() } else { pool.next_bid_order() };
