	...
	V: Clone + PartialOrd,
{
	...
	pub fn remove_leaf_by_index(&mut self, leaf_index: &K) -> Result<V, CritbitTreeError> {
		...
		let LeafNode { value, parent: parent_index, .. } =
			self.leaves.remove(leaf_index).expect("We already check above");
		if is_empty {
			self.reset()
		} else {
	...
	pub fn previous_leaf(&self, key: &K) -> Result<Option<(K, K)>, CritbitTreeError> {
		...
		let leaf_node =
			self.leaves.get(&leaf_index).ok_or(CritbitTreeError::LeafNodeShouldExist)?;
...
impl<K: OrderBookIndex, V> LeafNode<K, V> {
	...
	pub fn new(key: K, value: V) -> Self {
		LeafNode { parent: K::PARTITION_INDEX, key, value }
	}
}
	...
	Order: OrderInterface<Account, Unit, BlockNumber> + Clone + PartialOrd,
{
	...
	fn place_order(
		...
		if let Some(leaf_index) = self.find_leaf(&key)? {
			...
			let leaf_node =
				self.leaves.get_mut(&leaf_index).ok_or(CritbitTreeError::LeafNodeShouldExist)?;
			leaf_node.value.placed(order_id, owner, quantity, expired_at);
			Ok(())
		} else {
			// Insert new leaf node with new order
			self.insert(key, Order::new(order_id, owner.clone(), quantity, expired_at))?;
			Ok(())
		}
	}
